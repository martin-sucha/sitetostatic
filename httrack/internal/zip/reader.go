...
// LocalExtraField is same as f.Extra, but the data comes from the local file header instead of central directory.
func (f *File) LocalExtraField() ([]byte, error) {
	offset, size, err := f.findExtraData()
	if err != nil {
		return nil, err
	}
	buf := make([]byte, size)
	_, err = f.zipr.ReadAt(buf, f.headerOffset+offset)
	if err != nil {
		return nil, err
	}
	return buf, nil
...
// findExtraData does the minimum work to verify the file has a header
// and returns the offset and size of local extra data.
func (f *File) findExtraData() (int64, int64, error) {
	var buf [fileHeaderLen]byte
	if _, err := f.zipr.ReadAt(buf[:], f.headerOffset); err != nil {
		return 0, 0, err
	}
	...
	filenameLen := int(b.uint16())
	extraLen := int(b.uint16())
	return int64(fileHeaderLen + filenameLen), int64(extraLen), nil
}
